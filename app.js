...
// Post-process products to inject detailed benefits and features if not explicitly set
PRODUCTS.forEach(p => {
    ...
    if (p.benefits.length === 0) {
        if (p.name.includes("Hoodie")) {
            p.benefits = ["Exceptional warmth & plush softness", "Double-lined hood retains perfect shape", "Heavyweight pocket utility"];
            p.features = ["Dense organic fleece/French Terry", "Clean drawstring-free design", "Secure snug ribbing"];
        } else if (p.name.includes("T-Shirt") || p.name.includes("Tee")) {
            p.benefits = ["Ultra-soft combed organic cotton comfort", "Heavyweight structured drape", "Resists fading and shrinking"];
            p.features = [p.name.includes("Kids") ? "180 GSM organic cotton" : "280 GSM combed cotton", "Dropped shoulder street silhouette", "High-density graphic print"];
        } else if (p.name.includes("Set")) {
            p.benefits = ["Coordinated effortless look", "Breathable natural fabric movement", "Soft natural fabric texture"];
            p.features = ["Matching two-piece combo", "Relaxed fit casual styling", "Adjustable elastic waist drawcords"];
        } else {
            p.benefits = ["Ultimate everyday layering", "Cozy loops interior comfort", "Long lasting ribbed shape"];
            p.features = ["380 GSM loopback cotton Terry", "Loose drop shoulder style", "Tonal logo embroidery detail"];
        }
    }
});
            ...
            <div class="product-info">
                ...
                <div class="product-card-footer">
                    <div class="product-price-wrapper">
                        ...
                        ${product.comparePrice ? `<span class="product-compare-price">₹${product.comparePrice.toLocaleString('en-IN')}</span>` : ''}
                    </div>
                </div>
            </div>
        </div>
...
function renderProductPage(productId) {
    ...
    document.getElementById('sticky-bar-price').textContent = `₹${product.price.toLocaleString('en-IN')}`;
            ...
            <div class="product-detail-layout">
                ...
                <div class="product-info-column">
                    ...
                    <div class="detail-price-box">
                        <span class="detail-price">₹${product.price.toLocaleString('en-IN')}</span>
                        ${product.comparePrice ? `<span class="detail-compare-price">₹${product.comparePrice.toLocaleString('en-IN')}</span>` : ''}
                        <span class="badge badge-sale">SAVE ₹${(product.comparePrice - product.price).toLocaleString('en-IN')}</span>
                    </div>
                    ...
                    ${PRODUCTS.filter(p => p.id !== product.id).slice(0, 4).map(relProd => `
                        <div class="product-card">
                            ...
                            <div class="product-info">
                                ...
                                <div class="product-card-footer">
                                    <span class="product-price">₹${relProd.price.toLocaleString('en-IN')}</span>
                                </div>
                            </div>
                        </div>
                    `).join('')}
...
function updateCartUI() {
    ...
    if (state.discountCode === 'VOGUE10') {
        ...
        discountText.textContent = `-₹${discount.toLocaleString('en-IN')}`;
    } else {
        discountRow.style.display = 'none';
    }

    const total = subtotal - discount;
    totalText.textContent = `₹${total.toLocaleString('en-IN')}`;

    // Update shipping progress bar towards Free Shipping target of ₹1,500
        ...
        upsellContainer.innerHTML = `
            ...
            <div class="upsell-product">
                ...
                <div class="upsell-info">
                    ...
                    <span class="upsell-price">₹${upsellProduct.price.toLocaleString('en-IN')} <span style="text-decoration:line-through; font-size:0.7rem; color:var(--color-text-muted);">₹${upsellProduct.comparePrice.toLocaleString('en-IN')}</span></span>
                </div>
...
function updateShippingProgress(subtotal) {
    ...
    if (subtotal >= target) {
        progressText.innerHTML = `🎉 <strong>CONGRATULATIONS!</strong> You've unlocked <strong>FREE Express Shipping</strong>!`;
        progressBarFill.style.width = '100%';
    } else {
        const diff = target - subtotal;
        progressText.innerHTML = `Add <span>₹${diff.toLocaleString('en-IN')}</span> more for <strong>FREE Express Shipping</strong>!`;
...
function bindCheckoutEvents() {
    ...
    if (checkoutForm) {
        checkoutForm.onsubmit = (e) => {
            ...
            // Place Order mock success state
            const orderNum = `ORDER #UV-${Math.floor(100000 + Math.random() * 900000)}`;
...
function populateCheckoutSummary(directCheckoutItem = null) {
    ...
    container.innerHTML = items.map(item => `
        <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.85rem;">
            ...
            <span style="font-weight:600; color:var(--color-secondary);">₹${(item.product.price * item.quantity).toLocaleString('en-IN')}</span>
        </div>
    `).join('');
...
// STATIC CONTACT FORM SUBMIT MOCK
                    ...
                    <div id="tracking-results-box" style="margin-top: 40px; display: none;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 24px; position: relative; padding-left: 32px; border-left: 2px dashed var(--color-border-light); margin-left: 10px;">
                            <div style="position: relative;">
                                <div style="position: absolute; left: -43px; top: 2px; width: 20px; height: 20px; border-radius: 50%; background: var(--color-success); color: #FFF; display: flex; align-items: center; justify-content: center; font-size: 0.7rem;"><i class="fa-solid fa-check"></i></div>
                            ...
                            <div style="position: relative;">
                                <div style="position: absolute; left: -43px; top: 2px; width: 20px; height: 20px; border-radius: 50%; background: var(--color-accent); color: #FFF; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; box-shadow: 0 0 8px var(--color-accent);"><i class="fa-solid fa-spinner fa-spin"></i></div>
                            ...
                            <div style="position: relative; opacity: 0.5;">
                                <div style="position: absolute; left: -43px; top: 2px; width: 20px; height: 20px; border-radius: 50%; background: var(--color-border-light); color: var(--color-text-muted); display: flex; align-items: center; justify-content: center; font-size: 0.7rem;"><i class="fa-solid fa-clock"></i></div>
...
window.handleOrderTracking = function(e) {
    e.preventDefault();
    ...
    showToast(`Status updated for order ${orderId}`, "success");
};
